...
void
setup(AcquireRuntime* runtime, AcquireProperties* props)
{
    CHECK(runtime);
...
void
test(AcquireRuntime* runtime, AcquireProperties* props)
{
    ...
    ASSERT_EQ(uint32_t, "%d", props->video[0].camera.settings.shape.x, 3200);
    ASSERT_EQ(uint32_t, "%d", props->video[0].camera.settings.shape.y, 3200);

    // Trying to set the offset should have no effect because the current
    // shape prevents it.
    props->video[0].camera.settings.offset = { .x = 128, .y = 128 };
    OK(acquire_configure(runtime, props));
    ASSERT_EQ(uint32_t, "%d", props->video[0].camera.settings.offset.x, 0);
    ASSERT_EQ(uint32_t, "%d", props->video[0].camera.settings.offset.y, 0);
    ...
    OK(acquire_configure(runtime, props));
    ASSERT_EQ(uint32_t, "%d", props->video[0].camera.settings.shape.x, 1024);
    ASSERT_EQ(uint32_t, "%d", props->video[0].camera.settings.shape.y, 1024);
    ASSERT_EQ(uint32_t, "%d", props->video[0].camera.settings.offset.x, 128);
    ASSERT_EQ(uint32_t, "%d", props->video[0].camera.settings.offset.y, 128);

    // Now trying to set the shape should have no effect because the current
    // offset prevents it.
    props->video[0].camera.settings.shape = { .x = 3100, .y = 3100};
    OK(acquire_configure(runtime, props));
    ASSERT_EQ(uint32_t, "%d", props->video[0].camera.settings.shape.x, 1024);
    ASSERT_EQ(uint32_t, "%d", props->video[0].camera.settings.shape.y, 1024);
...
main()
{
    ...
    try {
        ...
        // After this test passes, the camera should have reasonable default
        // property values.
        test(runtime, &props);
